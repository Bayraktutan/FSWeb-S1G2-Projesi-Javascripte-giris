...
function cocukSarkisi(number){
  return number +" küçük maymun yatakta zıplamış, biri düşüp başını çarpmış, Anne doktoru aramış, Doktor çok kızmış: Bir daha yatakta zıplamak yok!"
}
...
function notHesapla(sayı){
  if (sayı>=90 && sayı<=100){
    return "A aldın"
  }else if(sayı>=80 && sayı<=89){
    return "B aldın"
  }else if(sayı>=70 && sayı<=79){
    return "C aldın"
  }else if(sayı>=60 && sayı<=69){
    return "D aldın"
  }else{
    return "F aldın"
  }
...
let counter = 0
function sesliHarfSayaci(vowelString) {
  for(letter of vowelString.toLowerCase()){
